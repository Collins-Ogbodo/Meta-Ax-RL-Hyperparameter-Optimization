...
def test_agent(env, model, max_steps=0):
  ...
  return ep_len, ep_rew, avg_step_time, eps_reward_metric_sum, eps_reward_metric_final
...
def evaluate_agent(env, model, steps_per_test, num_tests):
  ...
  for ep in range(num_tests):
    ep_len, ep_rew, step_time, reward_metric_sum, reward_metric_final = test_agent(env, model, max_steps=steps_per_test)
    avg_ep_len += ep_len
    avg_ep_rew += ep_rew
  ...
  return avg_ep_len, avg_ep_rew, avg_step_time, avg_reward_metric_sum, avg_reward_metric_final
...
class EvalAndSaveCallback(BaseCallback):
  ...
  def _on_step(self):
    if self.n_calls % self.check_freq == 0:
      ...
      # Evaluate the agent
      avg_ep_len, avg_ep_rew, avg_step_time, avg_reward_metric_sum, avg_reward_metric_final = evaluate_agent(
          env,
          self.model,
          self.steps_per_test,
          self.num_tests
...
settings = {
    'wandb_project': "TD3-cantilever-ax-hpo-Mode-1-2-3_2-Sensor-Baseline",
    'model_name': "TD3-cantilever-Mode-1-2-3_2-Sensor-Baseline",
    'ax_experiment_name': "TD3-cantilever-experiment-Mode-1-2-3_2-Sensor-Baseline",
    'ax_objective_name': "avg_ep_rew",
    'replay_buffer_name': "TD3-replay-buffer",
    'save_dir': "TD3-cantilever-ax-hpo-Mode-1-2-3_2-Sensor-Baseline-checkpoints",
...
MODEL_FILENAME = "checkpoints-Mode-1-2-3-evaluation/TD3-cantilever-Mode-1-2-3-evaluation_40960.zip"
# Create our environment
#try:
#  env.close()
#except NameError:
...
# Load the model
model = sb3.TD3.load(MODEL_FILENAME)
print('Stop Time', datetime.datetime.now(datetime.timezone.utc))
#%%
# Test the model
print('Start Time', datetime.datetime.now(datetime.timezone.utc))
ep_len, ep_rew, avg_step_time, eps_reward_metric_sum, eps_reward_metric_final = test_agent(
    env,
    model,
